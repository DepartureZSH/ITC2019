...
from MARL.HybridMAPPO.MAPPO.agents import agent_class
...
class CustomEnvironment:
    ...
    def initial_solution(self, solution_path, value_path=None):
        ...
        if value_path:
            with open(value_path, 'r') as f:
                values = json.load(f)
                self.agents_value = np.array([values[agent.id] for agent in self.agents])
                self.order_agents()
        self.penalty_benchmark = Total_cost
        mappo_observations, _ = self.agents_observe()
        return mappo_observations, masks, quality
    ...
    def reset(self, order=False):
        ...
        return mappo_observations, masks, none_assignment
