...
class MIPSolver:
    def __init__(self, reader, logger, config=None):
        ...
        self.fractional_count = defaultdict(int)
    ...
    #########################################################################################################
    # Setup from Scratch: build_model()
    #########################################################################################################
    ...
    #########################################################################################################
    # Setup from File: load_model(model_path)
    #########################################################################################################
    ...
    def model_callback(self, model, where):
        if where == GRB.Callback.MIPNODE:
            xvars = self.model.getVars()
            vals = self.model.cbGetNodeRel(xvars)
            for v, val in zip(xvars, vals):
                if v.VarName.startswith("x") and 1e-5 < val < 1 - 1e-5:
                    cid = v.VarName.split('_')[1]
                    self.fractional_count[cid] += 1
    ...
    def solve(self):
        ...
        self.model.optimize(self.model_callback)
    ...
    def save_model(self, output_path):
        ...
        fractional_count = self.fractional_count
        with open(f"{output_path}.json", 'w') as f:
            json.dump(self.fractional_count, f, indent=4)
    ...
    # Same Room but Time Overlap(Deprecated)
