...
class MAPPO:
    def __init__(self, team_size, state_dim, action_dim, config, output_folder):
        self.team_size = team_size
        self.gamma = config['train']['MAPPO']['mappo']['gamma']
        self.lmbda = config['train']['MAPPO']['mappo']['lmbda']
        self.eps = config['train']['MAPPO']['mappo']['eps']
        self.hidden_dim = config['train']['MAPPO']['mappo']['hidden_dim']
        self.actor_lr = float(config['train']['MAPPO']['mappo']['actor_lr'])
        self.critic_lr = float(config['train']['MAPPO']['mappo']['critic_lr'])
        self.device = config['train']['device']
        self.weights_dir = output_folder
    ...
    def save(self, pname):
        torch.save(self.actor.state_dict(), os.path.join(self.weights_dir, f"actor.pth"))
        torch.save(self.critic.state_dict(), os.path.join(self.weights_dir, f"acritic.pth"))

    def load(self, pname):
        actor_path = os.path.join(self.weights_dir, f"actor.pth")
        if os.path.exists(actor_path):
            self.actor.load_state_dict(torch.load(actor_path))
        critic_path = os.path.join(self.weights_dir, f"acritic.pth")
