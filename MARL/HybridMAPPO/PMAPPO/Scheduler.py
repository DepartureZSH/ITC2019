...
class Scheduler:
    def __init__(self, obs_dim, action_dim, config, output_folder):
        self.obs_dim = obs_dim
        self.action_dim = action_dim
        self.gamma = config['train']['PMAPPO']['sched']['gamma']
        self.lr = config['train']['PMAPPO']['sched']['learning_rate']
        self.max_epsilon = config['train']['PMAPPO']['sched']['max_epsilon']
        self.min_epsilon = config['train']['PMAPPO']['sched']['min_epsilon']
        self.decay_rate = config['train']['PMAPPO']['sched']['decay_rate']
    ...
    def save(self, pname):
        np.save(f'{self.output_dir}/{pname}.npy', self.Qtable)
    
    def load(self, pname):
        self.Qtable = np.load(f'{self.output_dir}/{pname}.npy')
