...
import json
import numpy as np
from MARL.MAPPO.Hierarchical2.agents import agent_class
from MARL.MAPPO.Hierarchical2.constraints import HardConstraints, SoftConstraints
    ...
    def move_agent(self, action_dict):
        for i, (cid, value) in enumerate(self.agents_order):
            if cid in self._assignment:
                ...
                self.agents_order[i] = (cid, (action_dict[cid]+1)/self.action_shape)
            else:
        ...
        self.agent_dict = {cid: i for i, (cid, _) in enumerate(agents_order)}
        self.agents_order = [(cid, i) if value!=0 else (cid, value) for i, (cid, value) in enumerate(agents_order)]
    ...
    def total_penalty(self):
        ...
        for agent in self.agents:
            ...
            if agent.id not in self._assignment:
                rewards += 1/agent.penalty if agent.penalty else 1
                penalty_agents[agent.id] = 1/agent.penalty if agent.penalty else 1
