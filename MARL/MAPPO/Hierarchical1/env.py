...
import json
import numpy as np
from MARL.MAPPO.Hierarchical1.agents import agent_class
from MARL.MAPPO.Hierarchical1.constraints import HardConstraints, SoftConstraints
...
class CustomEnvironment:
    ...
    def reset(self, seed: int | None = None):
        ...
        for agent in self.agents:
            ...
            agent.observe_space = np.array([agent.action_space[i][2] if i<len(agent.action_space) else 10 for i in range(self.obs_shape)], dtype=np.float64)
            observations[agent.id] = self.agent_observe(agent.action_space, 0)
        return observations
    
    def adjustment(self, unassign, seed: int | None = None):
        if seed is not None:
            np.random.seed(seed)
        self._assignment = []
        self._sched = []
        ...
        for agent in self.agents:
            ...
            agent.choice = 0
            agent.action_space = agent._actions()
            if agent.id in unassign:
                agent.value = agent.ajuster_action_space.sample()
            else:
                agent.value = len(agent.action_space)
            agent.observe_space = np.array([agent.action_space[i][2] if i<len(agent.action_space) else 10 for i in range(self.obs_shape)], dtype=np.float64)
            observations[agent.id] = self.agent_observe(agent.action_space, 0)
    ...
    # def adjustment(self, unassign):
        # for cid in unassign:
        #     i = self.cid2ind[cid]
        #     self.agents[i].value = self.agents[i].ajuster_action_space.sample()
    ...
    def agent_observe(self, actions, idx=0):
        obs = np.array([actions[i][2] if i<len(actions) else 10 for i in range(idx, idx + self.obs_shape)], dtype=np.float64)
        return obs
    ...
    def getConflicts(self, cid, action):
                ...
                if violate: 
                    for ccid in self.roomRelatedClass[room_option['id']]:
                        if cid != ccid:
                            ...
                            if self.agents[ci].action!=None:
                                confilct_agent_cid.add(ccid)
        return list(confilct_agent_cid)
    ...
    def agent_value_update(self, conflicts):
        flag = True
        # for ccid in self._agents:
        #     if ccid not in self._sched:
            ...
            if ccid not in self._sched:
                ...
                if agent.id not in self._sched:
                    ...
                    for aid, action in enumerate(agent.action_space):
                        ...
                        feasible, _ = self.is_feasible(agent.id, action)
                        if not feasible:
                            continue
                        else:
    ...
    def step_course(self, cid, action_idx):
        ...
        for aid, action in enumerate(agent.action_space):
            ...
            if not feasible:
                valid_actions_mask.append(0)
                continue
            penalty = self.incremental_penalty(cid, action)
            valid_actions_mask.append(1)
        ...
        if len(valid_actions) == 0:
            ...
            next_course_obs = np.ones_like(agent.observe_space, dtype=np.float64)*10

            # Scheduler 的下一个全局状态（这一步可以直接复用当前状态，或者让 env 内部记录“失败标志”）
            next_sched_state = self.get_scheduler_obs()

            return reward, next_sched_state, next_course_obs, done_course, no_valid
        ...
        # === 正常情况：有合法动作 ===
            ...
            if action_idx == 0:
                ...
                # chosen_action_space = spaces.Discrete(len(valid_actions), start=0, seed=42)
                action_idx = agent._action_space.sample(mask=np.array(valid_actions_mask, dtype=np.int8))
                chosen_action = agent.action_space[action_idx]
            # action_idx = action_idx % len(valid_actions)
            # chosen_action = valid_actions[action_idx]

            agent.candidate = chosen_action
            conflicts = self.getConflicts(cid, chosen_action)
            self.apply_action(cid, chosen_action, chosen_action[2])
            reward = 1 / chosen_action[2] if chosen_action[2] else 1
            done_course = True
            self._sched.append(cid)
            
            # self._assignment.append(cid)

            next_course_obs = self.agent_observe(valid_actions, action_idx)
            if not self.agent_value_update(conflicts):
                reward = - 10.0
            next_sched_state = self.get_scheduler_obs()
