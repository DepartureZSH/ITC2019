...
from MARL.MAPPO.Hierarchical1.env import CustomEnvironment
from MARL.MAPPO.Hierarchical1.network import MAPPO, Scheduler
    ...
    for episode in tqdm(range(total_episodes), desc=f"Training none assignment"):
        ...
        while True:
            ...
            while len(unsched) > 0:
                ...
                # 环境只对这门课执行一次排课
                reward, next_sched_state, next_course_obs, done_course, no_valid = env.step_course(
            ...
            print(f"run: {(iters)%len(agent_order)} {len(unassign)}/{len(agent_order)} has no valid option")
            if len(unassign) == 0:
                break
            else:
                obs = env.adjustment(unassign)
                unsched = env.get_unsched_courses()
                ep_reward = 0
                ep_sched_reward = 0
                ep_penalty = 0
        ...
        if valid:
            ...
            logger.info(f"Episode {episode+1} no assignment: {len(unassign)}")
