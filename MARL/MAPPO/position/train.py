...
from MARL.MAPPO.position.env import CustomEnvironment
from MARL.MAPPO.position.network import MAPPO
...
def plot_all_metrics(env_name, plots_dir, metrics_dict, episode, episodes_avg_num):
    """
...
def Position_MAPPO_train(reader, logger, fileName, config, epoches, quickrun=False):
    ...
    for episode in tqdm(range(total_episodes), desc=f"Training none assignment {len(done)}"):
        ...
        while len(done) > 0:
            ...
            ep_reward += np.sum(np.array([np.sum(next_obs[a]) for a in agent_order], dtype=np.float64))
            ep_penalty = penalty
            ...
            for i, agent in enumerate(agent_order):
                ...
                buffers[i]['rewards'].append(np.sum(next_obs[agent]))
        ...
        if episode % episodes_avg_num == 0:
            ...
            logger.info(f"Episode {episode}: "
                        f"AvgTotalReward(last50)={avg_reward:.3f}, "
                        f"AvgEpisodeLength(last50)={avg_length:.3f}, "
                        f"AvgPolicyLoss(last50)={avg_policy_loss:.3f}, "
                        f"AvgValueLoss(last50)={avg_value_loss:.3f}, "
                        f"AvgEntropy(last50)={avg_entropy:.3f}")
            ...
            # 调用新的绘图函数
            plot_all_metrics(env_name, output_dir, metrics_dict, episode, episodes_avg_num)
